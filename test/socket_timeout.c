...
void timeout_callback(socket_wrapper_t session)
{
    TEST_FALSE(session->activity_flag);
    session->closure_requested = true;
}
...
int main(void)
{
...

    {
        ...
        socket_session_set_timeout(server_session, 200, timeout_callback);
        ...
        // Wait for the timeout the connection
...

    {
        ...
        socket_session_set_timeout(client, 200, timeout_callback);
